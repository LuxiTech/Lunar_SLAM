"""D455 hardware profile: wide RGB-D driver, canonical relay, IMU and TF."""
...
def _as_bool(value, name: str) -> bool:
    if isinstance(value, bool):
        return value
    raise RuntimeError(f"The D455 config setting '{name}' must be true or false.")
...
def _launch_d455(context):
    config_path = LaunchConfiguration("config").perform(context)
    settings = _settings(config_path)
    configured_setup = str(settings.get("d455_setup", "")).strip()
    setup_path = Path(
        configured_setup
        or _workspace_root() / "device" / "D455" / "ros2_ws" / "install" / "setup.bash"
    ).expanduser()
    if not setup_path.is_file():
        raise RuntimeError(f"D455 setup script does not exist: {setup_path}")
    ...
    actions = [
        LogInfo(msg=f"Launching D455 driver using {setup_path}"),
        ExecuteProcess(
...
def generate_launch_description():
    return LaunchDescription([
        DeclareLaunchArgument(
            "config",
            description="D455 adapter configuration selected by sensor_bringup.launch.py.",
        ),
        OpaqueFunction(function=_launch_d455),
    ])
