...
### USB OFA/PVA/VIC 与学习特征建图（2026-08-10 最终版）

NX 实机确认深度后端为 `VPI OFA+PVA+VIC SGM`，没有进入 CPU 回退。USB `480x270`
SuperPoint 引擎使用 TensorRT FP16；LightGlue 使用 AMP FP16。生产位姿不再由独立双帧
PnP 累积：RTAB F2M 直接读取高频原子 RGB-D，以 5 帧局部 BA 发布 `/rtabmap/odom`；
Luxi 只以 1 Hz 向 RTAB-Map 提供 SuperPoint/LightGlue 外部描述子，且不发布 TF。

最终参数为单次 OFA、2 个 OpenCV 后处理线程、`RGBD/ProximityAngle=24` 和
`RGBD/OptimizeMaxError=3`。两次 OFA 与单 OpenCV 线程都使深度处理增至约 113 ms，
已由现场 A/B 否决。45 秒静止实测结果：

| 指标 | 最终 VPI + Luxi |
| --- | ---: |
| 里程计平均 / 中位频率 | 5.30 / 7.15 Hz |
| 末端 / 最大平移漂移 | 3.65 / 13.97 mm |
| 最大旋转漂移 | 0.436° |
| 学习前端跟踪 | 165/165 接受，0 丢失 |
| 平均内点 / 内点率 / RMSE | 246.0 / 75.56% / 0.700 px |
| GR3D 平均 / P90 / 峰值 | 58.7% / 87.2% / 97% |
| GR3D ≥90% | 8.6% |
| 最大单进程 CPU | 深度 0.885 核；其余 ≤0.611 核 |
| RAM | 7.47 GB |

对已移动实测库重放时，原始图约束为 12.5 mm 平移闭合、5.18° 横滚/俯仰合成残差；
24° 空间回环门限为 18.9 mm、0.66°，地图倾斜降低 87.2%。map048 稳定链的另一条
手持轨迹为 8.0 mm / 5.02°。两条轨迹没有动捕真值且手持终点不同，因此结论是 VPI
链已达到 D435i 量级的厘米级建图、姿态平整度更好，不能据此宣称绝对 ATE 更优。
