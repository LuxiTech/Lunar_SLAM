...
网页与导航先输出标准
当前控制数据使用 `192.168.123.0/24` 有线链路；网页仍可从 NX 的操作端局域网访问。
...
RGB 预览按 2 Hz 拉取，恢复融合 main 前的遥控优先级；浏览器页面隐藏时暂停拉取。当前 NX 为调试建图质量保留
`enable_cloud_preview=true`，需要优先保证遥控带宽时可关闭该选项，RTAB-Map 内部建图和
地图保存不受影响。
...
| `enable_cloud_preview` | `true` | 是否额外订阅并开放实时建图点云；带宽紧张时可关闭 |
...
- `GET /api/preview/cloud`：USB/CRE 建图默认启用；设置 `enable_cloud_preview=false`
  可关闭。手动控制期间返回 409，避免抢占控制链路带宽。
