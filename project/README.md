...
Intel RealSense D435i/D455 与 Stereolabs ZED X 替换接入；各设备经过 `luxi_adapter` 后使用相同的 RGB-D、
...
| 包 | 职责 |
| --- | --- |
| `luxi_adapter` | 选择 Hik/D435i/D455/ZED X 硬件 profile，发布统一 RGB-D、IMU 和 TF |
...
正式链路只在 `luxi_adapter` 启动时选择硬件。Hik、D435i、D455 与 ZED X 后面的视觉前端、RTAB、
...
同一时刻只能启动一个硬件 profile，不能让 Hik、D435i、D455 和 ZED X 同时发布统一传感器话题。
