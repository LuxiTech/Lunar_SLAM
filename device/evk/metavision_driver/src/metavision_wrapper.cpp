...
void MetavisionWrapper::setAntiFlicker(
  const bool state, const uint32_t lowFrequency, const uint32_t highFrequency)
{
  antiFlicker_.enabled = state;
  antiFlicker_.lowFrequency = lowFrequency;
  antiFlicker_.highFrequency = highFrequency;
}
...
void MetavisionWrapper::activateAntiFlicker()
{
  auto * antiFlicker =
    cam_.get_device().get_facility<Metavision::I_AntiFlickerModule>();
  if (!antiFlicker) {
    LOG_WARN_NAMED("this camera does not support anti-flicker filtering!");
    return;
  }
  if (antiFlicker_.enabled) {
    if (!antiFlicker->set_filtering_mode(
        Metavision::I_AntiFlickerModule::AntiFlickerMode::BAND_STOP)) {
      LOG_WARN_NAMED("cannot set anti-flicker mode to BAND_STOP!");
    }
    if (!antiFlicker->set_frequency_band(
        antiFlicker_.lowFrequency, antiFlicker_.highFrequency)) {
      LOG_WARN_NAMED(
        "cannot set anti-flicker frequency band to " << antiFlicker_.lowFrequency << "-"
                                                     << antiFlicker_.highFrequency << " Hz!");
    }
  }
  antiFlicker->enable(antiFlicker_.enabled);
  LOG_INFO_NAMED(
    "anti-flicker " << (antiFlicker_.enabled ? "enabled" : "disabled") << " in "
                    << antiFlicker_.lowFrequency << "-" << antiFlicker_.highFrequency << " Hz");
}
