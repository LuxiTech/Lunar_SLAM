...
void DriverROS2::configureWrapper(const std::string & name)
{
  ...
  bool antiFlicker;
  this->get_parameter_or("anti_flicker", antiFlicker, false);
  int antiFlickerLowFrequency;
  this->get_parameter_or("anti_flicker_low_frequency", antiFlickerLowFrequency, 90);
  int antiFlickerHighFrequency;
  this->get_parameter_or("anti_flicker_high_frequency", antiFlickerHighFrequency, 110);
  if (antiFlicker) {
    LOG_INFO(
      "Using anti-flicker BAND_STOP filter in " << antiFlickerLowFrequency << "-"
                                                << antiFlickerHighFrequency << " Hz");
    wrapper_->setAntiFlicker(
      antiFlicker, static_cast<uint32_t>(antiFlickerLowFrequency),
      static_cast<uint32_t>(antiFlickerHighFrequency));
  }
  bool trailFilter;
