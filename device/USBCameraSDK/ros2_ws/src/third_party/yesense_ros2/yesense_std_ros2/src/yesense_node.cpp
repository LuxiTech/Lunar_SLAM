...
class YESENSE_Publisher : public rclcpp::Node
{
	...
	: Node("yesense_publisher")
	{
		...
		this->declare_parameter<bool>("use_sample_timestamp", true);
		this->declare_parameter<double>("invalid_gyro_threshold_dps", 2100.0);
		this->declare_parameter<double>("invalid_accel_threshold_mps2", 200.0);
		...
		this->get_parameter("use_sample_timestamp", use_sample_timestamp_);
		this->get_parameter("invalid_gyro_threshold_dps", invalid_gyro_threshold_dps_);
		this->get_parameter("invalid_accel_threshold_mps2", invalid_accel_threshold_mps2_);
		this->get_parameter("drop_trigger_reset_sample", drop_trigger_reset_sample_);
		this->get_parameter(
		...
		RCLCPP_INFO(
			this->get_logger(),
			"IMU timestamp source: %s\n",
			use_sample_timestamp_ ? "H30 sample_timestamp (requires data ID 0x51)" : "host receive time");
	...
	void timer_callback()
	{
		...
		if(analysis_ok == ret)
		{
			...
			{ 
				...
				// anchoring, but do not expose that sample to downstream filters.
				if (drop_trigger_reset_sample_ &&
					yis_out.content.sample_timestamp &&
					yis_out.sample_timestamp == 0u)
				{
					RCLCPP_DEBUG_THROTTLE(
						this->get_logger(), *this->get_clock(), 5000,
						"Dropping H30 external-trigger reset sample");
					return;
				}
				publish_msg(&yis_out, stamp);
	...
	{
		const auto host_now = this->now();
		if (!use_sample_timestamp_ || !result->content.sample_timestamp)
		{
			if (use_sample_timestamp_ && !missing_sample_timestamp_warned_)
			{
				RCLCPP_WARN(
					this->get_logger(),
					"H30 sample_timestamp (data ID 0x51) is absent; falling back to host receive time");
				missing_sample_timestamp_warned_ = true;
			}
			return host_now;
		}

		const uint32_t sample_timestamp_us = result->sample_timestamp;
		const auto sample_offset = rclcpp::Duration::from_nanoseconds(
			static_cast<int64_t>(sample_timestamp_us) * 1000LL);

		// H30 resets the sample timestamp to zero after every external SYNC
		// pulse.  Re-anchor each period and use device microsecond offsets for
		// all following 200 Hz samples, which removes serial receive jitter.
		if (!sample_epoch_valid_ || sample_timestamp_us < last_sample_timestamp_us_)
		{
			sample_epoch_ros_ = host_now - sample_offset;
			sample_epoch_valid_ = true;
		}

		last_sample_timestamp_us_ = sample_timestamp_us;
		auto stamp = sample_epoch_ros_ + sample_offset;
		// The host anchor can have sub-millisecond UART scheduling jitter at a
		// SYNC reset. Keep ROS timestamps strictly monotonic for downstream
		// filters while preserving the device-time spacing within each period.
		if (last_imu_stamp_valid_ && stamp <= last_imu_stamp_)
		{
			stamp = last_imu_stamp_ + rclcpp::Duration::from_nanoseconds(1);
		}
	...
	void publish_msg(yis_out_data_t *result, const rclcpp::Time &stamp);
	...
	bool use_sample_timestamp_{true};
	double invalid_gyro_threshold_dps_{2100.0};
...
void YESENSE_Publisher::publish_msg(yis_out_data_t *result, const rclcpp::Time &stamp)
{
    ...
    // =========== publish imu message ==============
    // This message is a class member reused at the output rate. Reset it so
    // an unavailable covariance from one bad packet cannot leak into a later
    // valid packet.
    ...
    {
        // IMU-only mode has no attitude estimate. ROS uses -1 to mark an
        // unavailable orientation rather than treating 0,0,0,0 as valid.
        imu_ros_data.orientation_covariance[0] = -1.0;
    }
 
    const bool gyro_valid = result->content.gyro &&
        std::isfinite(result->gyro.x) && std::isfinite(result->gyro.y) &&
        std::isfinite(result->gyro.z) &&
        std::abs(result->gyro.x) < invalid_gyro_threshold_dps_ &&
        std::abs(result->gyro.y) < invalid_gyro_threshold_dps_ &&
        std::abs(result->gyro.z) < invalid_gyro_threshold_dps_;
    if (gyro_valid)
    {
    ...
    else
    {
        ...
        if (invalidate_orientation_with_invalid_gyro_)
        {
            // The H30 quaternion is generated from the same gyro stream. A
            // saturated axis makes that fused attitude unsafe as a visual
            // odometry prior even when a finite quaternion is present.
            imu_ros_data.orientation_covariance[0] = -1.0;
        }
        RCLCPP_WARN_THROTTLE(
            this->get_logger(), *this->get_clock(), 5000,
            "H30 gyro is unavailable or saturated (%.3f, %.3f, %.3f dps); "
            "marking angular velocity%s unavailable",
            result->gyro.x, result->gyro.y, result->gyro.z,
            invalidate_orientation_with_invalid_gyro_ ? " and orientation" : "");
    }
    const bool accel_valid = result->content.acc &&
        std::isfinite(result->acc.x) && std::isfinite(result->acc.y) &&
        std::isfinite(result->acc.z) &&
        std::abs(result->acc.x) < invalid_accel_threshold_mps2_ &&
        std::abs(result->acc.y) < invalid_accel_threshold_mps2_ &&
        std::abs(result->acc.z) < invalid_accel_threshold_mps2_;
    if (accel_valid)
    {
    ...
    else
    {
        ...
        RCLCPP_WARN_THROTTLE(
            this->get_logger(), *this->get_clock(), 5000,
            "H30 acceleration is unavailable or saturated (%.3f, %.3f, %.3f m/s^2); "
            "marking linear acceleration unavailable",
