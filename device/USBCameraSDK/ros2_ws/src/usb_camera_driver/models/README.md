...
The launch file uses `crestereo_init_iter2_180x320_fp16.onnx` here. The model is
the Apache-2.0 CREStereo pre-trained weight converted to fixed-shape ONNX by
PINTO0309's model zoo; no project-specific training or fine-tuning is used.
...
The optional `crestereo_init_iter2_360x640.onnx` is the quality profile used
for offline comparison. It is not the NX default because its CUDA execution
